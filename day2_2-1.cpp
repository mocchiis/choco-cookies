...
// 1つ目のキューブ
	...
	// 2つ目のキューブ
	glPushMatrix();//現在の変換行列を保存.これがないと1つ目のキューブに設定が+されてしまう。
	glTranslatef(-1.0f, 0.0f, 0.0f);//(dx,dy,dz)だけ移動
	glRotatef(1.0f * theta, 0.0f, 1.0f, 0.0f);// (angle,vx,vy,vz)ベクトル(vx,vy,vz)を回転軸としてangleだけ回転
	glScalef(0.5f, 0.5f, 0.5f);
