...
static const char *WIN_TITLE = "2-3Quaternion";     // ウィンドウのタイトル
...
void drawCubeQuaternion(float theta, float x, float y, float z, float *mat) {
	//glRotatef(theta, 0.0f, 1.0f, 0.0f)の代わり;
	float norm = sqrtf(x * x + y * y + z * z);
	x /= norm;
	y /= norm;
	z /= norm;

	float radian = theta * (PI / 360.0f) ;
	glm::quat q(cos(radian), x*sin(radian), y*sin(radian), z*sin(radian));//回転を表す
...
// キューブ
	...
	float mat[16] = { 0 };
	drawCubeQuaternion(theta, 1.0f, 0.0f, 0.0f, mat);
	glMultMatrixf(mat);
